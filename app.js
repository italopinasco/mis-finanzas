...
function calculateAccountBalance(account){
  ...
  // Ingresos y gastos: solo se consideran si la cuenta fue seleccionada/vinculada.
  movements.forEach(m=>{
  ...
  // Transferencias: salida de origen y entrada al destino.
...
function populateMovementAccountSelectors(){
  ["incomeAccount","expenseAccount"].forEach(id=>{
    ...
    el.innerHTML='<option value="">Selecciona una cuenta</option>'+
      accounts.map(a=>`<option value="${a.nombre.replace(/"/g,"&quot;")}">${accountLabel(a)}</option>`).join("");
    if(accounts.some(a=>a.nombre===current)) el.value=current;
  });
}
